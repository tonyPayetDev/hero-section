...
## ⚠️ Dépendance manquante (reconstruction 2026-07-06)

Ce skill appelle des tools `mcp__coolify__*` (`list_applications`, `application`, `projects`, `deploy`, `get_application`, `application_logs`, `deployment`). Ce serveur MCP Coolify n'est **plus connecté** dans cette session (`claude mcp list` ne le montre pas, aucune trace dans `~/.claude.json` ni ses sauvegardes, aucun dossier `/coolify` sur le disque) — probablement perdu dans le même crash VPS qu'`avatar-reel` (dernière trace d'usage réel : note projet du 2026-06-30). Ce fichier ne fonctionnera pas tant que ce serveur MCP n'est pas reconnecté (`claude mcp add coolify ...` avec la config d'origine, si elle peut être retrouvée). Une variante sans dépendance MCP (curl direct vers l'API REST Coolify) existe dans le repo public `tonyPayetDev/kilo-skills/coolify-deploy` — ⚠️ ce repo contenait un vrai token d'accès Coolify en clair dans son `SKILL.md` au moment de la consultation (2026-07-06), à faire tourner et retirer de l'historique git si ce n'est pas déjà fait.
