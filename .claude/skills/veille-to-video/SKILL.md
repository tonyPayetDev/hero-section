...
**Piège vérifié (2026-08-19, autoboost-62 Claude Sonnet 5) : un badge de statut à l'intérieur d'un conteneur "coloré" (ex. `.steprail.good .node`) peut perdre silencieusement son propre `background` face à une règle de teinte du conteneur, même déclarée AVANT lui dans le fichier, à cause de la spécificité CSS — pas de l'ordre.** Repéré sur un diagramme d'automatisation en 2 rangées de nœuds (`.steprail.bad .node{background:rgba(255,95,87,.08)}` / `.steprail.good .node{background:rgba(40,200,64,.08)}`, 3 classes de spécificité via le sélecteur descendant `.conteneur.variante .node`) plus une variante ponctuelle sur le dernier nœud (`.node.win{background:var(--good)}`, seulement 2 classes). Le sélecteur à 3 classes l'emporte sur celui à 2 classes indépendamment de l'ordre d'écriture dans le CSS — le badge final ("30+ ✓") héritait donc du fond à 8% d'opacité du rail au lieu du vert plein, avec un texte `color` volontairement très sombre (pour contraster sur un fond plein) devenu illisible sur ce fond quasi noir. `validate`/`inspect` ne l'ont pas détecté (aucune erreur de contraste WCAG déclenchée, le calcul se fait sur les couleurs *déclarées* dans la règle qui gagne, pas sur un rendu visuellement "vide"). Repéré uniquement en extrayant une frame réelle au milieu de la fenêtre où le badge est censé être visible et en zoomant dessus (`ffmpeg -vf crop=...` + lecture des pixels). Fix : `!important` sur le `background` des variantes spéciales (`.node.win`, `.node.brk`), ou augmenter leur spécificité pour qu'elle dépasse celle du sélecteur de teinte du conteneur (ex. `.steprail .node.win`). Toujours vérifier par pixel/frame réelle tout élément dont la couleur de fond dépend d'une variable CSS et qui vit à l'intérieur d'un conteneur ayant lui-même une règle de teinte sur `.node`/l'élément enfant générique.
